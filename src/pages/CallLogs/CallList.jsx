...
import callLogs from "./call-log-data";
...
const CallList = ({ active, setActive }) => {
  return (
    <section className="rounded-3xl color-card pb-6 ">
      ...
      {callLogs.map((item) => (
        <div
          className={`flex justify-between p-3 sm:p-4 relative ${item.id === active.id ? "border-b-3 border-[#2B7FFF]" : item.id === callLogs[callLogs.length - 1].id ? "" : "border border-[#2B7FFF10]  cursor-pointer "} `}
          key={item.id}
          onClick={() => setActive(item)}
        >
          <div className="flex flex-col gap-3 sm:gap-5 grw">
